...
def restart_services():
    """Send reload command to datalogger worker (disabled in webapp-only mode)"""
    try:
        # In webapp-only mode, we cannot directly communicate with external processes
        # The external datalogger process should handle config changes automatically
        print("INFO: Config changes will be picked up by external datalogger process")
    except Exception as e:
        print(f"INFO: restart_services disabled in webapp-only mode: {e}")
