...
# Fallback unpacker nếu không có value_converter
...
class TCPWorker:
    def __init__(self, worker_id, host, port=502, timeout=2.0,
        ...
        # Modbus client
        self.client = None
        self._unit_kw = "unit"  # auto-detect
    ...
    def _loop(self):
        if self.debug:
            print(f"🔄 TCP loop for {len(self.devices)} devices")
        schedule_next = {}
    ...
    def _read_block(self, device, fc, tg_list):
        ...
        # số regs thật sự cần đọc phải theo datatype max
        # dùng converter để tính số regs; nếu không có thì ước lượng 1
        max_regs = 1
        for t in tg_sorted:
            ...
            if VC_AVAILABLE:
                ...
                max_regs = max(max_regs, 2 if dt in ("float","float32","real","int32","uint32","dint","dword","long") else 1)
                if dt in ("double","float64"): max_regs = max(max_regs, 4)

        # để an toàn block bao phủ đủ cho tag dài nhất
